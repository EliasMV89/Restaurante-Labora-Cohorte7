...
func main() {
	...
	for {
		...
		case 1:
			...
			nuevaMesa := servicios.Mesa{
				NumeroDeMesa: numero,
				Capacidad:    capacidad,
				Disponible:   1,
			}
			servicios.AgregarMesa(db, nuevaMesa)
		case 2:
		...
		case 3:
			...
			var cantidad int
			fmt.Scanln(&cantidad)
			nuevaReserva := servicios.Reserva{
				ClienteID:          clienteId,
				MesaID:             mesaId,
				FechaReserva:       fecha,
				HoraReserva:        hora,
			...
			servicios.AgregarReserva(db, nuevaReserva)
			servicios.ActualizarMesa(db, 1, mesaId)
		case 4:
			fmt.Println("Analizar patrones de reserva")
			fmt.Printf("Ingrese la fecha a analizar: ")
			var fecha string
			fmt.Scanln(&fecha)
			servicios.AnalizarReserva(db, fecha)
		case 5:
			os.Exit(0)
		default:
